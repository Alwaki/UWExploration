...
# Load first model
model1 = pickle.load(open(r"/home/alex/.ros/" + name + ".pickle","rb"))
...
ind_points = model1.model.variational_strategy.inducing_points.cpu().detach().numpy()
...
inputsg = [
    np.linspace(104.5, 170, n),
    np.linspace(-1237.5, -1184.5, n)
]
...
variance = np.clip(np.vstack(var_list).reshape(s), 0.5, 2.8)
...
#plt.savefig("/home/alex/Pictures/tree_gp_plot/" + name + ".png")
#plt.show()
