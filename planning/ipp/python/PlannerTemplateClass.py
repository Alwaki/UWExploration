...
class PlannerTemplate(object):
    ...
    def __init__(self, corner_topic, path_topic, planner_req_topic, odom_topic, bounds, 
        ...
        # Setup timer to store result
        rospy.Timer(rospy.Duration(10), self.timer_save_model)
    ...
    def timer_save_model(self, msg):
        """ Periodically saves the trained model, for evaluation purposes.
            Uses pickle model directly, not safe for usage on some devices.
        """
        with self.gp.mutex:
            with open(self.store_path  + "_GP_env_vis_" + str(round(self.distance_travelled)) + ".pickle", "wb") as fp:
                pickle.dump(self.gp.model, fp)
                print("pickled! at distance " + str(round(self.distance_travelled)))
                with open('path.npy', 'wb') as f:
                    np.save(f, np.array(self.state_history))
                print("Saved history of path, overwrote old path.")
